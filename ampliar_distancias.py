...
def calcular_distancias(fich_rios, contaminante_sf, fich_output, contaminante_col):
    for idx, rio in fich_rios.iterrows():
        ...
        for part in geometry.geoms:
            #Se reducen el número de tramos a la mitad
            simplified_part = part.simplify(tolerance=0.5, preserve_topology=False)
            num_segments_simplified = len(simplified_part.coords)
            #print(f"Número de tramos del rio simplificados: {num_segments_simplified}")
            for i in range(num_segments_simplified):
            # Obtener las coordenadas de cada punto
                coordinates = simplified_part.coords[i]
                #print("Calculando el sensor más cercano al punto: ",coordinates)
                distances = []
                for idx, row in contaminante_sf.iterrows():
                    sensor = row.geometry.coords[0]  # Obtenemos las coordenadas del sensor
                    distance = np.sqrt((coordinates[0] - sensor[0]) ** 2 + (coordinates[1] - sensor[1]) ** 2)
...
# FICHEROS OUTPUT
contaminacion_fitobentos = "/Users/silviadelatorre/Desktop/TFG/DISTANCIAS SENSORES/Valor_fitobentos_py.csv"
contaminacion_nitrato = "/Users/silviadelatorre/Desktop/TFG/DISTANCIAS SENSORES/Valor_Nitratos_py.csv"
contaminacion_amonio = "/Users/silviadelatorre/Desktop/TFG/DISTANCIAS SENSORES/Valor_amonio_py.csv"
contaminacion_fosforo = "/Users/silviadelatorre/Desktop/TFG/DISTANCIAS SENSORES/Valor_fosforo_py.csv"
contaminacion_fosfato = "/Users/silviadelatorre/Desktop/TFG/DISTANCIAS SENSORES/Valor_fosfato_py.csv"
contaminacion_grado_trofico = "/Users/silviadelatorre/Desktop/TFG/DISTANCIAS SENSORES/Valor_grado_trofico_py.csv"

contaminantesResult_list = [(fitobentos_gdf,contaminacion_fitobentos,"Fitobentos"),(nitrato_gdf,contaminacion_nitrato,"Nitrato"),(amonio_gdf,contaminacion_amonio,"Amonio"),(fosforo_gdf,contaminacion_fosforo,"Fosforo"),(fosfato_gdf,contaminacion_fosfato,"Fosfato"),(grado_trofico_gdf,contaminacion_grado_trofico,"Grado Trofico")]

# Obtener el tiempo de inicio
start_time = time.time()

# for vertiente in fich_rios_list:
#     for contaminante_sf, output, contaminante_col in contaminantesResult_list:
#         print(contaminante_col)
#         calcular_distancias(vertiente, contaminante_sf, output,contaminante_col)
